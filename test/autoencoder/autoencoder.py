...
num_hidden_2 = 64 #128 # 2nd layer num features (the latent dim)
...
def encoder(x):
    ...
    with tf.name_scope("encoder") as scope:
        layer_1 = tf.nn.sigmoid(tf.add(tf.matmul(x, weights['encoder_h1']),
                                       biases['encoder_b1']), name='layer_1')
        # Encoder Hidden layer with sigmoid activation #2
        layer_2 = tf.nn.sigmoid(tf.add(tf.matmul(layer_1, weights['encoder_h2']),
                                       biases['encoder_b2']), name='layer_2')
    return layer_2
...
def decoder(x):
    ...
    with tf.name_scope("decoder") as scope:
        layer_1 = tf.nn.sigmoid(tf.add(tf.matmul(x, weights['decoder_h1']),
                                       biases['decoder_b1']), name='layer_1')
        # Decoder Hidden layer with sigmoid activation #2
        layer_2 = tf.nn.sigmoid(tf.add(tf.matmul(layer_1, weights['decoder_h2']),
                                       biases['decoder_b2']), name='layer_2')
    return layer_2

def test():
    #a = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
    #print('GLOBAL_VARIABLES', a)
    a = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
    print('TRAINABLE_VARIABLES:')
    for t in a:
        print(t.name)
    #a = tf.get_collection(tf.GraphKeys.MODEL_VARIABLES)
    #print('MODEL_VARIABLES', a)
